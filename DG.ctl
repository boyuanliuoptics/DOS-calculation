...
;when there is time-reversial (T) symmetry we can use half of the Brillouin zone (BZ), that is z ranges in [0,num-k/2]
;the default is computing without T-symmetry. change 'false' to 'true' if you want to reduce half of BZ 
(define-param T-symmetry? false)
...
;the default 'false' is GGR method. change 'false' to 'true' if you want to use Tr method.
(define-param GGR-Tr? true)

;num-k is the inter number of k points along one dimension when sampling the BZ
(define-param num-k 10); num-k should be even number to make the mesh avoid Gamma point when using GGR method
...
(if GGR-Tr?
	...
	(set! k-shift k-shift-GGR); GGR k points (default)
)
...
(lambda (z)
	...
	(if T-symmetry?
		(interpolate num-k (list (- k-shift 0) (- kx k-shift))); half of the BZ
		(interpolate num-k (list (- k-shift kx) (- kx k-shift))); the whole BZ (default)
	)
)
))
